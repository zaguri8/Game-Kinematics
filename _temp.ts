...
function translatePoint(p: vec2, center: vec2) {
...
function rotatePoint(p: vec2, tetha: number) {
    const pOut = vec2.rotate(vec2.clone(p), p, vec2.fromValues(0, 0), -(tetha * Math.PI) / 180);
    return pOut;
}

function reflect(theta: number) {
    const cosTwoTheta = Math.cos(2 * theta);
    const sinTwoTheta = Math.sin(2 * theta);
    const result = mat2d.fromValues(
        cosTwoTheta, sinTwoTheta,
        sinTwoTheta, -cosTwoTheta,
        0,
        0
    );
...
function reflectTriangle(p1: vec2, p2: vec2, p3: vec2, tetha: number = 90) {
    const reflection = reflect(tetha * (Math.PI) / 180);
    let p1Out = vec2.create(),
        p2Out = vec2.create(),
        p3Out = vec2.create();
    ...
    return [p1Out, p2Out, p3Out] as const
}



function plotTriangle(context: CanvasRenderingContext2D, p1: vec2, p2: vec2, p3: vec2, color: vec3 = vec3.fromValues(0, 0, 0)) {
    plotLine(context, p1, p2, color);
    plotLine(context, p2, p3, color);
    plotLine(context, p3, p1, color);
}

function plotLine(context: CanvasRenderingContext2D, start: vec2, end: vec2, color: vec3 = vec3.fromValues(0, 0, 0)) {
    context.beginPath();
